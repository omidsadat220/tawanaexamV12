...
<html lang="en" dir="ltr">
  <head>
    ...
    <style>
      ...
      * { font-family: "Inter", sans-serif; }
      ...
      .gradient-border::before { content: ""; position: absolute; top: 0; left: 0; right: 0; bottom: 0; border-radius: 16px; padding: 2px;
      ...
      @keyframes float { 0% { transform: translateY(100vh) rotate(0deg); opacity: 0;} 10% { opacity: 1;} 90% { opacity: 1;}
  ...
  <body class="min-h-screen py-8 px-4">
    ...
    <div id="imageModal" class="image-modal">
      ...
      <button onclick="closeModal()" class="absolute top-4 right-4 text-white text-2xl bg-gray-800 p-2 rounded-full hover:bg-gray-700">
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-gray-800 rounded-2xl p-6 relative overflow-hidden">
          ...
          <div class="flex flex-col md:flex-row justify-between items-center relative z-10">
            ...
            <div>
              ...
              <p class="text-gray-400 mt-2">Specialized Web Programming Questions {{$category->uni_name}}</p>
            </div>
      ...
      <form method="POST" action="{{ route('exam.submit') }}">
        ...
        <div class="gradient-border">
          <div class="bg-gray-800 rounded-2xl p-6 md:p-8 relative overflow-hidden">
            ...
            @foreach($answers as $item)
              <div class="gradient-border mb-8">
                <div class="bg-gray-900 rounded-xl p-6">
                  ...
                  <!-- Options -->
                  <div class="space-y-4">
                    <label class="option-hover flex items-center bg-gray-800 rounded-xl p-4 cursor-pointer">
                      <input type="radio" name="answers[{{ $item->id }}]" value="{{ $item->question_one }}" class="hidden" />
                    ...
                    <label class="option-hover flex items-center bg-gray-800 rounded-xl p-4 cursor-pointer">
                      <input type="radio" name="answers[{{ $item->id }}]" value="{{ $item->question_two }}" class="hidden" />
                      <div class="w-6 h-6 rounded-full border-2 border-gray-600 flex items-center justify-center mr-3">
                    ...
                    <label class="option-hover flex items-center bg-gray-800 rounded-xl p-4 cursor-pointer">
                      <input type="radio" name="answers[{{ $item->id }}]" value="{{ $item->question_three }}" class="hidden" />
                      <div class="w-6 h-6 rounded-full border-2 border-gray-600 flex items-center justify-center mr-3">
                        <div class="w-3 h-3 rounded-full bg-purple-500 opacity-0"></div>
                      </div>
                    ...
                    <label class="option-hover flex items-center bg-gray-800 rounded-xl p-4 cursor-pointer">
                      <input type="radio" name="answers[{{ $item->id }}]" value="{{ $item->question_four }}" class="hidden" />
                      <div class="w-6 h-6 rounded-full border-2 border-gray-600 flex items-center justify-center mr-3">
                        <div class="w-3 h-3 rounded-full bg-purple-500 opacity-0"></div>
                      </div>
            ...
            <div class="flex justify-center mt-8">
              <button type="submit" class="px-6 py-3 bg-gradient-to-r from-green-600 to-dark-600 text-white rounded-lg hover:from-dark-700 hover:to-green-700 transition-all duration-300 flex items-center pulse">
    ...
    <script>
      ...
      document.querySelectorAll("label").forEach((label) => {
        label.addEventListener("click", function () {
          ...
          parent.querySelectorAll(".option-hover div > div").forEach((el) => { el.style.opacity = "0"; });
